...
const {
  Product,
  validateproduct,
} = require("../../models/product/product.schema");
...
module.exports.add = async (req, res) => {
  try {
    ...
    const add = new Product({
      ...
      product_partner_id:
        req.body.product_partner_id == undefined ||
        req.body.product_partner_id == ""
          ? null
          : req.body.product_partner_id,
    ...
    if (save) {
      return res
        .status(200)
        .json({ message: "เพิ่มสินค้าสำเร็จ", data: save, status: true });
    } else {
      return res
        .status(400)
        .json({ message: "เพิ่มสินค้าไม่สำเร็จ", status: false });
    }
  } catch (error) {
...
module.exports.getall = async (req, res) => {
  try {
    const get = await Product.find().populate({
      path: "product_partner_id",
      select: "partner_name partner_company_name",
    });
    if (get) {
      return res
        .status(200)
        .json({ message: "ดึงข้อมูลสินค้าสำเร็จ", data: get, status: true });
    } else {
      return res
        .status(400)
        .json({ message: "ดึงข้อมูลสินค้าไม่สำเร็จ", status: false });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message, status: false });
  }
...
module.exports.getbyid = async (req, res) => {
  try {
    const get = await Product.findById(req.params.id).populate({
      path: "product_partner_id",
      select: "partner_name partner_company_name",
    });
    if (get) {
...
module.exports.search = async (req, res) => {
  try {
    ...
    if (get) {
      return res
        .status(200)
        .json({ message: "ค้นหาสินค้าสำเร็จ", data: get, status: true });
    } else {
      return res
        .status(400)
        .json({ message: "ค้นหาสินค้าไม่สำเร็จ", status: false });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message, status: false });
  }
...
module.exports.edit = async (req, res) => {
  try {
    ...
    if (!check) {
      return res
        .status(400)
        .json({ message: "ไม่พบข้อมูลสินค้า", status: false });
    }
    ...
    if (edit) {
      return res
        .status(200)
        .json({ message: "แก้ไขสินค้าสำเร็จ", data: edit, status: true });
    } else {
      return res
        .status(400)
        .json({ message: "แก้ไขสินค้าไม่สำเร็จ", status: false });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message, status: false });
  }
...
module.exports.delete = async (req, res) => {
  try {
    ...
    if (del) {
      ...
      return res
        .status(400)
        .json({ message: "ลบสินค้าไม่สำเร็จ", status: false });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message, status: false });
  }
...
module.exports.status = async (req, res) => {
  try {
    ...
    const status = await Product.findByIdAndUpdate(
      req.params.id,
    ...
    if (status) {
      return res
        .status(200)
        .json({
          message: "เปลี่ยนสถานะสินค้าสำเร็จ",
          data: status,
          status: true,
        });
    } else {
      return res
        .status(400)
        .json({ message: "เปลี่ยนสถานะสินค้าไม่สำเร็จ", status: false });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message, status: false });
  }
...
module.exports.addimgproduct = async (req, res) => {
  try {
    ...
    upload(req, res, async function (err) {
      ...
      const edit = await Product.findByIdAndUpdate(req.params.id, data, {
        new: true,
      });
      return res
        .status(200)
...
module.exports.addimagedoc = async (req, res) => {
  try {
    let upload = multer({ storage: storage }).array("image", 20);
    upload(req, res, async function (err) {
      const reqFiles = [];
      ...
      if (req.files) {
        ...
        const product = await Product.findById(req.params.id);
        if (product.product_imagedoc != "") {
          deleteimage(product.product_imagedoc);
        }
      } else {
        return res.json({
      ...
      const data = { product_imagedoc: image };
      const edit = await Product.findByIdAndUpdate(req.params.id, data, {
        new: true,
      });
      return res
...
module.exports.getbypartner = async (req, res) => {
  try {
    const get = await Product.find({
      product_partner_id: req.params.id,
    }).populate({
      path: "product_partner_id",
      select: "partner_name partner_company_name",
    });
  ...
  try {
    ...
    if (typecode == "shop") {
      token = jwt.sign(
        ...
        process.env.SHOP_SECRET_KET
      );
    } else if (typecode == "service") {
      token = jwt.sign(
        { code: "service", name: "service", key: "service" },
        process.env.SHOP_SECRET_KET
      );
    } else if (typecode == "office") {
      token = jwt.sign(
        { code: "office", name: "office", key: "office" },
        process.env.SHOP_SECRET_KET
      );
    } else {
      return res
        .status(400)
        .json({ message: "ไม่พบ typecode ที่ต้องการ", status: false });
    }
    return res
      .status(200)
      .json({ message: "สร้าง token สำเร็จ", data: token, status: true });
  } catch (error) {
    return res.status(500).json({ message: error.message, status: false });
  }
};
...
module.exports.gettruestatus = async (req, res) => {
  try {
    const get = await Product.find({ product_status: true }).populate({
      path: "product_partner_id",
      select: "partner_name partner_company_name",
    });
    if (get) {
