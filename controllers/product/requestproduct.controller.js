...
const {
  Requestproduct,
} = require("../../models/product/requestproduct.schema");
...
module.exports.add = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: "คำร้องขอฝากขายสินค้าสำเร็จ", data: add });
  } catch (err) {
...
module.exports.getAll = async (req, res) => {
  try {
    const data = await Requestproduct.find().populate({
    ...
    return res
      ...
      .json({
        status: true,
        message: "ข้อมูลคำร้องขอฝากขายสินค้าทั้งหมด",
        data: data,
      });
  } catch (err) {
    return res.status(500).json({ status: false, message: err.message });
...
module.exports.getById = async (req, res) => {
  try {
    const data = await Requestproduct.findById(req.params.id).populate({
      path: "product_partner_id",
      select: "partner_name partner_company_name",
    });
    return res
      ...
      .json({
        status: true,
        message: "ข้อมูลคำร้องขอฝากขายสินค้า",
        data: data,
      });
  } catch (err) {
    return res.status(500).json({ status: false, message: err.message });
...
module.exports.update = async (req, res) => {
  try {
    ...
    if (checrequest) {
      ...
      const edit = await Requestproduct.findByIdAndUpdate(req.params.id, data, {
        new: true,
      });
      return res
        .status(200)
        .json({
          status: true,
          message: "อัพเดทข้อมูลคำร้องขอฝากขายสินค้าสำเร็จ",
          data: edit,
        });
    }
    return res
      .status(400)
      .json({ status: false, message: "ไม่พบข้อมูลคำร้องขอฝากขายสินค้า" });
  } catch (err) {
    return res.status(500).json({ status: false, message: err.message });
  }
};
...
module.exports.delete = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        status: true,
        message: "ลบข้อมูลคำร้องขอฝากขายสินค้าสำเร็จ",
        data: data,
      });
  } catch (err) {
    return res.status(500).json({ status: false, message: err.message });
...
module.exports.approve = async (req, res) => {
  try {
    ...
    const data = await Requestproduct.findByIdAndUpdate(
      req.params.id,
      {
        request_status: true,
        request_status_detail: checkrequest.request_status_detail,
        office_id: office_id,
    ...
    const product = new Product({
      ...
      product_basecost: data.product_basecost,
      ...
      product_imagedoc: data.product_imagedoc,
    ...
    if (data && add) {
      return res
        ...
        .json({
          status: true,
          message: "อนุมัติคำร้องขอฝากขายสินค้าสำเร็จ",
          data: data,
        });
    } else {
      return res
        ...
        .json({
          status: false,
          message: "ไม่สามารถอนุมัติคำร้องขอฝากขายสินค้าได้",
        });
    }
  } catch (err) {
    return res.status(500).json({ status: false, message: err.message });
...
module.exports.disapprove = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        status: true,
        message: "ไม่อนุมัติคำร้องขอฝากขายสินค้าสำเร็จ",
        data: data,
      });
  } catch (err) {
    console.log(err);
